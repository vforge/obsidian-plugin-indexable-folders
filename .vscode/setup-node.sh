...
if [ "$NVM_LOADED" = true ]; then
    ...
    # Check for pnpm and fall back to nvm if pnpm is not available
    echo ""
    echo "🔎 Checking for pnpm..."
    if command -v pnpm >/dev/null 2>&1; then
        echo "✅ pnpm is available: $(pnpm --version 2>/dev/null)"
    else
        echo "⚠️  pnpm not found. Attempting to use nvm to select Node.js version (so pnpm can be installed if needed)..."

        # If nvm is available, try to use the version from .nvmrc
        if type nvm >/dev/null 2>&1; then
            if [ -f .nvmrc ]; then
                echo "Using nvm to switch Node.js version from .nvmrc..."
                nvm use || echo "⚠️  nvm use failed"
            else
                echo "⚠️  No .nvmrc file found; cannot run 'nvm use' to provide Node.js for pnpm"
            fi
        else
            echo "⚠️  nvm is not available; pnpm is not installed or on PATH"
        fi
    fi
